...
Let's check out the two primary categorical values: rideable_type, and member_casual, and count the number of values based on a few different groupings.
...
```
So far, the data has no noticeable errors, and is easy to work with.
...
ggplot(data=df, mapping=aes(x=start_lat)) +
  geom_histogram()
```
Since the distribution of latitudes is so visibly narrow, we can assume that our distance conversions will be more than accurate enough for the purposes of this analysis. Therefore, let's create a new variable, ride_distance, in miles. 
<br><br>
...
df2 <- df2[,c(1, 10, 11, 12, 2, 3, 5, 7, 4, 6, 8, 9)]
head(df2)
```
